  ...
  return (
    <div className="min-h-[100vh] bg-transparent w-full max-w-7xl mx-auto px-6 lg:px-12 py-8 transition-colors">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 xl:grid-cols-[360px_1fr] gap-6">
        ...
        {/* Past Sessions List */}
        <Card className="col-span-1 p-7 rounded-[1.5rem] shadow-sm border border-slate-200/60 bg-white min-h-[350px] flex flex-col hover:shadow-md transition-shadow">
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-base font-bold text-[#1c1c1c] flex items-center gap-2.5">
              <LucideHistory className="w-4 h-4 text-slate-400" /> Past Sessions
            </h3>
            <span className="text-[10px] uppercase font-black text-slate-500 bg-[#f8f7f5] border border-slate-200/80 px-2.5 py-1 rounded-md">{closedSessions.length} total</span>
          </div>
          <div className="flex-1 space-y-3 overflow-y-auto custom-scrollbar pr-3">
             {closedSessions.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-center py-10 opacity-70">
                   ...
                   <div className="text-xs font-bold text-slate-400">No session history yet.</div>
                </div>
             ) : (
                closedSessions.map((session) => {
                   ...
                   return (
                      <button
                         ...
                         className={
                            'w-full text-left rounded-[1.25rem] p-4 border transition-all text-xs group ' +
                            (isActive ? 'border-[#5c7c6d] bg-[#fdfdfc] shadow-[0_4px_15px_rgba(92,124,109,0.08)]' : 'border-slate-100 bg-[#fbfaf8] hover:bg-white hover:border-slate-200 hover:shadow-sm')
                         }
                      >
                         <div className="flex items-center justify-between mb-2">
                            <div className={"font-bold text-[14px] truncate pr-2 " + (isActive ? 'text-[#1c1c1c]' : 'text-[#1c1c1c] group-hover:text-[#5c7c6d] transition-colors')}>{getSessionName(session)}</div>
                            <div className={"text-[10px] font-black px-2 py-0.5 rounded-md shadow-sm border " + (isActive ? 'bg-[#5c7c6d] text-white border-[#4a6357]' : 'bg-white border-slate-100 text-slate-500')}>
                               {session.attendanceCount || session.attendances?.length || 0}
                            </div>
                         </div>
                         <div className="text-[10px] uppercase tracking-widest font-bold opacity-60 flex items-center gap-1.5">
                            <Clock className="w-3 h-3" /> {formatDate(getSessionStart(session))}
                         </div>
                      </button>
                   )
                })
...
<div className="flex bg-[#ffffff] rounded-[1.25rem] border border-slate-200/60 p-1.5 shadow-[0_4px_25px_rgba(0,0,0,0.03)] mb-8 relative z-10 w-full overflow-hidden">
 ...
 = '<Card className="col-span-1 p-7 rounded-[1.5rem] shadow-sm border border-slate-200/60 bg-white h-[450px] flex flex-col hover:shadow-md transition-shadow">
            <div className="flex items-center justify-between mb-5 border-b border-slate-100 pb-4">
              <h3 className="text-[17px] font-black tracking-tight text-[#1c1c1c] flex items-center gap-2.5">
                <LucideHistory className="w-4.5 h-4.5 text-slate-400" /> Past Sessions
              </h3>
...
.Replace(, <div className="flex bg-[#ffffff] rounded-[1.25rem] border border-slate-200/60 p-1.5 shadow-[0_4px_25px_rgba(0,0,0,0.03)] mb-8 relative z-10 w-full overflow-hidden">
            <div className="flex-1 text-center py-3 relative group hover:bg-[#f4f7f6]/60 transition-colors rounded-xl cursor-default">
               <div className="flex items-center justify-center gap-1.5 text-[10px] md:text-[11px] uppercase tracking-widest text-[#5c7c6d] font-black mb-1.5 opacity-90"><CheckCircle className="w-3.5 h-3.5" /> Present</div>
               <div className="text-[34px] md:text-[40px] font-black text-[#5c7c6d] leading-none tracking-tighter drop-shadow-sm">{activePresentCount}</div>
               <div className="absolute right-0 top-1/4 bottom-1/4 w-[1px] bg-slate-200/70"></div>
