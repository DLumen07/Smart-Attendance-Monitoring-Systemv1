...
<Card className="col-span-1 lg:col-span-2 xl:col-span-1 p-7 rounded-[1.5rem] shadow-[0_4px_25px_rgba(0,0,0,0.02)] border border-slate-200/60 min-h-[350px] max-h-[500px] flex flex-col bg-white hover:shadow-[0_8px_30px_rgba(0,0,0,0.04)] transition-shadow relative overflow-hidden">
            ...
            {selectedSession ? (
                  ...
                  <div className="flex-1 overflow-y-auto custom-scrollbar pr-3 space-y-2 rounded-xl bg-white min-h-[0]">
                     {attendance.length === 0 ? (
                        <div className="flex flex-col items-center justify-center h-full text-center py-8">
                           ...
                           <div className="text-[13px] text-slate-400 font-bold">No attendance records found for this session.</div>
                        </div>
                     ) : (
                        attendance.map((record, index) => {
                           const student = record.student || {}
               ...
               <div className="flex flex-col items-center justify-center flex-1 min-h-[250px] text-center opacity-70 mt-8">
                  <div className="w-16 h-16 rounded-full bg-[#f8f7f5] border border-slate-200 flex items-center justify-center mb-4 inner-shadow-[inset_0_2px_10px_rgba(0,0,0,0.02)]">
                     <FolderDot className="w-8 h-8 text-slate-300" />
                  </div>
                  <div className="text-[15px] font-black text-slate-400">Select a past session</div>
                  <p className="text-[12px] text-slate-400/80 mt-1.5 font-medium max-w-[220px] mx-auto">Click any session from the history list to analyze its records.</p>
               </div>
            )} = '<div className="flex flex-wrap md:flex-nowrap items-stretch bg-[#ffffff] border border-slate-200/70 rounded-[1.25rem] p-1.5 mb-5 shadow-[0_8px_30px_rgba(0,0,0,0.03)] shrink-0">
                     <div className="flex-1 flex items-center gap-3.5 px-4 py-2 relative group cursor-default">
                        <div className="w-[34px] h-[34px] rounded-full bg-[#f4f7f6] text-[#5c7c6d] flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform">
                           <ClipboardList className="w-4 h-4" />
                        </div>
                        <div className="min-w-0 pr-2">
                           <div className="text-[9px] uppercase tracking-widest text-slate-400 font-black mb-0.5">Session Name</div>
                           <div className="font-bold text-[14px] text-[#1c1c1c] truncate">{getSessionName(selectedSession)}</div>
                        </div>
                        <div className="hidden md:block absolute right-0 top-1/4 bottom-1/4 w-[1px] bg-slate-100"></div>
                     </div>
                     <div className="flex-1 flex items-center gap-3.5 px-4 py-2 relative group cursor-default">
                        <div className="w-[34px] h-[34px] rounded-full bg-slate-50 text-slate-500 border border-slate-100 flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform">
                           <Calendar className="w-4 h-4 opacity-70" />
                        </div>
                        <div className="min-w-0 pr-2">
                           <div className="text-[9px] uppercase tracking-widest text-slate-400 font-black mb-0.5">Date</div>
                           <div className="font-bold text-[14px] text-[#1c1c1c] truncate">{formatDate(getSessionStart(selectedSession))}</div>
                        </div>
                        <div className="hidden md:block absolute right-0 top-1/4 bottom-1/4 w-[1px] bg-slate-100"></div>
                     </div>
                     <div className="flex-1 flex items-center gap-3.5 px-4 py-2 relative group cursor-default">
                        ...
                        <div className="min-w-0 pr-2">
                           <div className="text-[9px] uppercase tracking-widest text-slate-400 font-black mb-0.5">Time</div>
                           <div className="font-bold text-[14px] text-[#1c1c1c] truncate">{formatTime(getSessionStart(selectedSession))}</div>
                        </div>
                     </div>
                     <div className="flex-[0.8] flex items-center justify-center gap-3 px-5 py-2.5 bg-gradient-to-br from-[#f4f7f6] to-[#ebf1ef] rounded-xl ml-1 border border-[#5c7c6d]/10 text-[#5c7c6d] shadow-[inset_0_2px_10px_rgba(255,255,255,0.7)]">
                        <div className="flex items-center gap-1.5 opacity-60">
                           <Users className="w-[18px] h-[18px]" />
                        </div>
                        <div className="flex items-baseline gap-1.5">
                           <span className="font-black text-[22px] leading-none tracking-tighter drop-shadow-sm">{attendance.length}</span>
                           <span className="text-[9px] font-black uppercase tracking-widest opacity-70">Students</span>
                        </div>
                     </div>
                  </div>'
