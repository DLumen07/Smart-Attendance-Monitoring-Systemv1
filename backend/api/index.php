...
function sendEmailMessage(array $config, string $to, string $subject, string $htmlBody, ?string $textBody, ?string &$errorMessage = null): bool
{
    ...
    if (!ensureMailerLoaded($errorMessage)) {
        return false;
    }
...
function ensureMailerLoaded(?string &$errorMessage = null): bool
{
    ...
    foreach ($required as $file) {
        if (!is_file($base . $file)) {
            $errorMessage = 'PHPMailer not found on server';
            return false;
        }
    }
    ...
    if (!class_exists('PHPMailer\\PHPMailer\\PHPMailer')) {
        $errorMessage = 'PHPMailer failed to load';
        return false;
    }
